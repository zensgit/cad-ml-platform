...
class CostEstimator:
    ...
    def _load_config(self):
        ...
        try:
            ...
            logger.info(f"Successfully loaded manufacturing data from {CONFIG_PATH}")
        except FileNotFoundError:
            logger.warning(f"Config file not found at {CONFIG_PATH}. Using fallback hardcoded defaults.")
            self._set_default_hardcoded_config()
        except yaml.YAMLError as e:
            logger.error(f"Error parsing YAML config at {CONFIG_PATH}: {e}. Using fallback hardcoded defaults.")
            self._set_default_hardcoded_config()
        except Exception as e:
            logger.error(f"Unexpected error loading config from {CONFIG_PATH}: {e}. Using fallback hardcoded defaults.")
            self._set_default_hardcoded_config()

    def _set_default_hardcoded_config(self):
        """Sets hardcoded default values if config loading fails."""
        self.materials_config = {
            "steel": {"price_per_kg_usd": 2.5, "density_kg_per_m3": 7850},
            "aluminum": {"price_per_kg_usd": 4.0, "density_kg_per_m3": 2700},
            "titanium": {"price_per_kg_usd": 40.0, "density_kg_per_m3": 4500},
            "plastic": {"price_per_kg_usd": 15.0, "density_kg_per_m3": 1200},
    ...
    def estimate(
        ...
        stock_weight_kg = weight_kg / (1.0 - min(0.9, removal_ratio)) if removal_ratio < 1.0 else weight_kg * 1.2
