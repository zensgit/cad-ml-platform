...
class AIProcessRecommender:
    ...
    def _load_config(self):
        ...
        try:
            ...
            logger.info(f"Successfully loaded process recommendation thresholds from {CONFIG_PATH}")
        except FileNotFoundError:
            logger.warning(f"Config file not found at {CONFIG_PATH}. Using fallback hardcoded defaults for process recommendation.")
            self._set_default_hardcoded_thresholds()
        except yaml.YAMLError as e:
            logger.error(f"Error parsing YAML config at {CONFIG_PATH}: {e}. Using fallback hardcoded defaults for process recommendation.")
            self._set_default_hardcoded_thresholds()
        except Exception as e:
            logger.error(f"Unexpected error loading config from {CONFIG_PATH}: {e}. Using fallback hardcoded defaults for process recommendation.")
            self._set_default_hardcoded_thresholds()
    ...
    def recommend(
        ...
        if complexity_score > self.proc_rec_thresholds.get("complex_geometry_score", 5.0) or \
           stock_removal > self.proc_rec_thresholds.get("high_stock_removal_additive", 0.9):
            rec = {
                "process": "additive_manufacturing",
                "method": "SLS" if "nylon" in material.lower() else "DMLS",
                "confidence": 0.85,
                "reason": "High geometric complexity and high material removal rate favors additive."
            }
        ...
        # Standard prismatic parts, moderate removal
            ...
            rec = {
                "process": "cnc_milling",
                "method": "3_axis" if complexity_score < self.proc_rec_thresholds.get("prismatic_complexity_max", 2.0) else "5_axis",
                "confidence": 0.90,
