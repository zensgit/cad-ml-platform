...
class DFMAnalyzer:
    ...
    def _load_config(self):
        ...
        try:
            ...
            logger.info(f"Successfully loaded DFM thresholds from {CONFIG_PATH}")
        except FileNotFoundError:
            logger.warning(f"Config file not found at {CONFIG_PATH}. Using fallback hardcoded defaults for DFM.")
            self._set_default_hardcoded_thresholds()
        except yaml.YAMLError as e:
            logger.error(f"Error parsing YAML config at {CONFIG_PATH}: {e}. Using fallback hardcoded defaults for DFM.")
            self._set_default_hardcoded_thresholds()
        except Exception as e:
            logger.error(f"Unexpected error loading config from {CONFIG_PATH}: {e}. Using fallback hardcoded defaults for DFM.")
            self._set_default_hardcoded_thresholds()
    ...
    def analyze(self, dfm_features: Dict[str, Any], part_type: str) -> Dict[str, Any]:
        ...
        if dfm_features.get("thin_walls_detected"):
            ...
            threshold = self.dfm_thresholds.get("min_wall_thickness_mm", 0.8) # Default if not in config
            if min_thk < threshold:
                issues.append({
                    "severity": "high",
                    "code": "THIN_WALL",
                    "message": f"Potential thin walls detected (~{min_thk:.2f}mm). Below threshold ({threshold:.2f}mm). May cause warping or printing failures."
                })
                score -= 20
        ...
        if removal_ratio > threshold:
            issues.append({
                "severity": "medium",
                "code": "HIGH_WASTE",
                "message": f"Requires removing {removal_ratio*100:.1f}% of stock material. Above threshold ({threshold*100:.1f}%). Consider casting or additive manufacturing."
            })
            score -= 10
        ...
        if part_type == "shaft" and ar > threshold:
             issues.append({
                "severity": "medium",
                "code": "SLENDER_PART",
                "message": f"Part is very slender (L/D = {ar:.1f}). Above threshold ({threshold:.1f}). Risk of vibration/deflection during turning."
            })
             score -= 15
        ...
        return {
            ...
            "issues": issues,
            "manufacturability": "high" if score > 80 else "medium" if score > 50 else "low"
        }
