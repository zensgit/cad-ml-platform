...
from src.api.health_resilience import CircuitState, resilience_collector
...
resilience_retry_attempts = Counter(
    "resilience_retry_attempts", "Total number of retry attempts", ["service", "endpoint"]
)
...
class ResilienceManager:
    ...
    def get_or_create_circuit_breaker(self, key: str, config: ResilienceConfig) -> CircuitBreaker:
        """获取或创建熔断器"""
    ...
    def get_or_create_rate_limiter(
        self, key: str, config: ResilienceConfig
    ) -> TokenBucketRateLimiter:
        """获取或创建限流器"""
        if key not in self.rate_limiters:
            rl = TokenBucketRateLimiter(rate=config.rl_rate, burst=config.rl_burst)
    ...
    def get_or_create_retry_policy(self, key: str, config: ResilienceConfig) -> RetryPolicy:
        """获取或创建重试策略"""
    ...
    def get_or_create_bulkhead(self, key: str, config: ResilienceConfig) -> SemaphoreBulkhead:
        """获取或创建隔离舱"""
        if key not in self.bulkheads:
            self.bulkheads[key] = SemaphoreBulkhead(
                max_concurrent_calls=config.bh_max_concurrent, timeout=config.bh_timeout
            )
    ...
    def update_metrics(self, config: ResilienceConfig):
        ...
        labels = {"service": config.service_name, "endpoint": config.endpoint_name}
...
def with_resilience(config: Optional[ResilienceConfig] = None, **kwargs) -> Callable:
    """
    主韧性装饰器 - 应用所有启用的韧性组件
            ...
            try:
                ...
                if rate_limiter and not rate_limiter.acquire():
                    resilience_rate_limit_rejected.labels(
                        service=config.service_name, endpoint=config.endpoint_name
                    ).inc()
                ...
                if bulkhead:
                    with bulkhead:
                        return _execute_with_circuit_breaker_and_retry(
                            func, args, kwargs, circuit_breaker, retry_policy, config
                        )
                else:
                    return _execute_with_circuit_breaker_and_retry(
                        func, args, kwargs, circuit_breaker, retry_policy, config
                ...
                if config.metrics_enabled:
                    resilience_request_duration.labels(
                        service=config.service_name, endpoint=config.endpoint_name, status=status
                    ).observe(duration)
