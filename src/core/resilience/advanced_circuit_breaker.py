"""Advanced circuit breaker (vision-independent core).

Extracted from src/core/vision/circuit_breaker.py by Phase 0 slice A2a so that the
sole real consumer -- src/core/dedupcad_vision.py, the HTTP client of the
dedupcad-vision service -- no longer imports anything from src/core/vision/.

That decoupling is the mandated precondition (design doc section 60 special case)
for Phase 0 slice 2, which prunes the ~106-decorator vision zoo; without it,
pruning vision/ would break the platform's only live integration.

Contains ONLY the generic breaker machinery, verbatim. The two VisionProvider-
decorating symbols stay in src/core/vision/circuit_breaker.py (they need .base).
"""
import threading
import time
