...
async def cache_tuning_recommendation(api_key: str = Depends(get_api_key)):
    ...
    try:
        ...
        feature_cache_tuning_recommended_capacity.set(result["recommended_capacity"])
        feature_cache_tuning_recommended_ttl_seconds.set(
            result["recommended_ttl_seconds"]
        )
        feature_cache_tuning_requests_total.labels(status="ok").inc()
    except Exception:
        pass

    return CacheTuningRecommendation(**result)
