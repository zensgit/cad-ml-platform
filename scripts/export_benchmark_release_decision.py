...
def build_release_decision(
    *,
    ...
    if (
        knowledge_domain_validation_matrix_component.get("status")
        not in {"", "unknown", "knowledge_domain_validation_ready"}
    ):
        for item in knowledge_domain_validation_matrix_recommendations:
            if item not in review_signals:
                review_signals.append(item)
    review_signals.extend(
        item
...
def render_markdown(payload: Dict[str, Any]) -> str:
    lines = [
        ...
        f"- `knowledge_domain_validation_matrix_status`: "
    ...
    if knowledge_validation_domains:
        ...
        lines.append("- none")
    knowledge_validation_recommendations = (
        payload.get("knowledge_domain_validation_matrix_recommendations") or []
    )
    if knowledge_validation_recommendations:
        for item in knowledge_validation_recommendations:
            lines.append(f"- recommendation: {item}")
