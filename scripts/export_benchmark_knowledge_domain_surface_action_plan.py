#!/usr/bin/env python3
"""Export benchmark knowledge domain surface action plan signals."""
...
def build_summary(
    *,
    ...
    component = build_knowledge_domain_surface_action_plan(
        knowledge_domain_surface_matrix_summary
    )
    return {
        "title": title,
        "generated_at": int(time.time()),
        "knowledge_domain_surface_action_plan": component,
        "recommendations": knowledge_domain_surface_action_plan_recommendations(
            component
        ),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Export benchmark knowledge domain surface action plan signals."
    )
    parser.add_argument(
        "--title",
        default="Benchmark Knowledge Domain Surface Action Plan",
    )
    ...
    payload = build_summary(
        title=args.title,
        knowledge_domain_surface_matrix_summary=_maybe_load_json(
    ...
    if args.output_md:
        _write_output(
            args.output_md,
            render_knowledge_domain_surface_action_plan_markdown(payload, args.title),
        )
    print(rendered)
