#!/usr/bin/env python3
"""Track E Slice E1 (dry-run): leakage-safe evaluation split + split artifact.

Ratified contract: L3_TRACK_E_EVALUATION_INTEGRITY_V2_DESIGNLOCK_20260721.md (#531).
Reference-only prior art: closed #510/#511 (not a merge base). Reimplemented on main.

Standalone split-integrity tooling. Fully DECOUPLED from the L3 gate
(``scripts/eval_integrity_gate.py`` is unconditional — no pass path — and this module
loads nothing from that module): the artifact built here carries a hardcoded
``unlocks_retraining: false`` and can never re-enable retraining. The integrity core
...
def _cell_text(value: object) -> str:
    """Normalize a manifest cell: None / blank → empty string (never the literal ``'None'``)."""
    if value is None:
        return ""
...
def _family_key(row: dict) -> str:
    ...
    return f"file:{normalized_family(_cell_text(row.get('file_path')) or str(row.get('file_path', '') or ''))}"
...
def compute_split(
    ...
    *,
    ...
    for row in rows:
        # None must not stringify to "None" (would silently keep a missing label / family).
        fp = _cell_text(row.get("file_path"))
        label = _cell_text(row.get("taxonomy_v2_class"))
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        # Missing manifest, unreadable paths — surface as integrity finding (clean exit 1).
        sys.stderr.write(f"[track-e] DRY-RUN reproducibility/integrity finding: {exc}\n")
        return 1
    return 2
