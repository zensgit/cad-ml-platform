#!/usr/bin/env python3
"""Export benchmark knowledge-domain surface-matrix signals."""
...
def build_summary(*, title: str) -> Dict[str, Any]:
    component = build_knowledge_domain_surface_matrix()
    return {
        "title": title,
        "generated_at": int(time.time()),
        "knowledge_domain_surface_matrix": component,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Export benchmark knowledge-domain surface matrix signals."
    )
    parser.add_argument(
        "--title",
        default="Benchmark Knowledge Domain Surface Matrix",
    )
    parser.add_argument("--output-json", default="")
    parser.add_argument("--output-md", default="")
    args = parser.parse_args()

    payload = build_summary(title=args.title)
    ...
    if args.output_md:
        _write_output(
            args.output_md,
            render_knowledge_domain_surface_matrix_markdown(payload, args.title),
        )
    print(rendered)
