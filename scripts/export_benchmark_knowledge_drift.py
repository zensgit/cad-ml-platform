#!/usr/bin/env python3
"""Export benchmark knowledge-readiness drift against a previous baseline."""
...
def build_knowledge_drift_summary(
    *,
    title: str,
    ...
    component = build_knowledge_drift_status(current_summary, previous_summary)
    return {
        "title": title,
        "generated_at": int(time.time()),
        "knowledge_drift": component,
        "recommendations": knowledge_drift_recommendations(component),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Export benchmark knowledge drift summary."
    )
    parser.add_argument("--title", default="Benchmark Knowledge Drift")
    ...
    payload = build_knowledge_drift_summary(
        title=args.title,
    ...
    if args.output_md:
        _write_output(
            args.output_md,
            render_knowledge_drift_markdown(payload, args.title),
        )
    print(rendered)
