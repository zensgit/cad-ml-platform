#!/usr/bin/env python3
"""Export benchmark knowledge source action-plan signals."""
...
def build_summary(
    *,
    ...
    component = build_knowledge_source_action_plan(knowledge_source_coverage_summary)
    return {
        "title": title,
        "generated_at": int(time.time()),
        "knowledge_source_action_plan": component,
        "recommendations": knowledge_source_action_plan_recommendations(component),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Export benchmark knowledge source action-plan signals."
    )
    parser.add_argument(
        "--title",
        default="Benchmark Knowledge Source Action Plan",
    )
    ...
    payload = build_summary(
        title=args.title,
        knowledge_source_coverage_summary=_maybe_load_json(
            args.benchmark_knowledge_source_coverage
        ),
        artifact_paths={
            "benchmark_knowledge_source_coverage": args.benchmark_knowledge_source_coverage,
        },
    )
    ...
    if args.output_md:
        _write_output(
            args.output_md,
            render_knowledge_source_action_plan_markdown(payload, args.title),
        )
    print(rendered)
