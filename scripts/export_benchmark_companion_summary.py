...
def _knowledge_source_drift_summary(component: Dict[str, Any]) -> str:
    if not component:
        return ""
    parts = [f"status={component.get('status') or 'unknown'}"]
    current_status = str(component.get("current_status") or "").strip()
...
def _competitive_surpass_trend_summary(component: Dict[str, Any]) -> str:
    ...
    regressions = _compact(component.get("pillar_regressions") or [])
    improvements = _compact(component.get("pillar_improvements") or [])
    new_gaps = _compact(component.get("new_primary_gaps") or [])
    resolved_gaps = _compact(component.get("resolved_primary_gaps") or [])
    if regressions:
...
def _knowledge_domain_release_gate(
    benchmark_knowledge_domain_release_gate: Dict[str, Any],
) -> Dict[str, Any]:
    gate = (
        benchmark_knowledge_domain_release_gate.get("knowledge_domain_release_gate")
        or benchmark_knowledge_domain_release_gate
        or {}
    )
    return {
        "status": str(gate.get("status") or "unknown"),
        "summary": str(gate.get("summary") or "none"),
        ...
        "releasable_domains": list(gate.get("releasable_domains") or []),
        "blocked_domains": list(gate.get("blocked_domains") or []),
        "priority_domains": list(gate.get("priority_domains") or []),
        "recommended_first_action": dict(gate.get("recommended_first_action") or {}),
    }


def _knowledge_domain_release_readiness_action_plan(
    benchmark_knowledge_domain_release_readiness_action_plan: Dict[str, Any],
) -> Dict[str, Any]:
    plan = (
        benchmark_knowledge_domain_release_readiness_action_plan.get(
            "knowledge_domain_release_readiness_action_plan"
        )
        or benchmark_knowledge_domain_release_readiness_action_plan
        or {}
    )
    return {
        "status": str(plan.get("status") or "unknown"),
        "total_action_count": int(plan.get("total_action_count") or 0),
        "high_priority_action_count": int(plan.get("high_priority_action_count") or 0),
        "medium_priority_action_count": int(
            plan.get("medium_priority_action_count") or 0
        ),
        "priority_domains": list(plan.get("priority_domains") or []),
        "actions": list(plan.get("actions") or []),
        "recommended_first_actions": list(plan.get("recommended_first_actions") or []),
    }


def _knowledge_reference_inventory(
    ...
    return {
        ...
        "summary": (
            f"domains={inventory.get('ready_domain_count', 0)}/"
            f"{inventory.get('total_domain_count', 0)} ready, "
            f"references={inventory.get('total_reference_items', 0)}, "
            f"tables={inventory.get('populated_table_count', 0)}/"
            f"{inventory.get('total_table_count', 0)}"
        ),
        "priority_domains": list(inventory.get("priority_domains") or []),
        "domains": dict(inventory.get("domains") or {}),
        "focus_tables_detail": list(inventory.get("focus_tables_detail") or []),
        "total_reference_items": int(inventory.get("total_reference_items") or 0),
    }
...
def _component_statuses(
    ...
    benchmark_knowledge_subdomain_surface_matrix: Dict[str, Any] | None = None,
    knowledge_domain_capability_drift_summary: Dict[str, Any] | None = None,
    ...
    knowledge_domain_release_readiness_action_plan_summary: Dict[str, Any] | None = None,
    ...
    knowledge_source_drift_summary: Dict[str, Any] | None = None,
    knowledge_outcome_correlation_summary: Dict[str, Any] | None = None,
    knowledge_outcome_drift_summary: Dict[str, Any] | None = None,
    knowledge_domain_release_surface_alignment_summary: Dict[str, Any] | None = None,
    ...
    knowledge_domain_capability_matrix_summary = (
        knowledge_domain_capability_matrix_summary or {}
    )
    benchmark_knowledge_domain_api_surface_matrix = (
        benchmark_knowledge_domain_api_surface_matrix or {}
    )
    ...
    benchmark_knowledge_subdomain_surface_matrix = (
        benchmark_knowledge_subdomain_surface_matrix or {}
    )
    knowledge_domain_capability_drift_summary = (
        knowledge_domain_capability_drift_summary or {}
    )
    knowledge_domain_action_plan_summary = knowledge_domain_action_plan_summary or {}
    knowledge_domain_control_plane_summary = (
        knowledge_domain_control_plane_summary or {}
    )
    knowledge_domain_control_plane_drift_summary = (
    ...
    knowledge_domain_release_readiness_action_plan_summary = (
        knowledge_domain_release_readiness_action_plan_summary or {}
    )
    ...
    knowledge_source_drift_summary = knowledge_source_drift_summary or {}
    ...
    knowledge_domain_release_surface_alignment_summary = (
        knowledge_domain_release_surface_alignment_summary or {}
    )
    ...
    knowledge_domain_capability_matrix_component = (
        knowledge_domain_capability_matrix_summary.get(
            "knowledge_domain_capability_matrix"
        )
        or knowledge_domain_capability_matrix_summary
        or {}
    )
    knowledge_domain_api_surface_matrix_component = (
        benchmark_knowledge_domain_api_surface_matrix.get(
            "knowledge_domain_api_surface_matrix"
        )
        or benchmark_knowledge_domain_api_surface_matrix
        or {}
    )
    knowledge_domain_surface_matrix_component = (
        benchmark_knowledge_domain_surface_matrix.get(
    ...
    knowledge_subdomain_surface_matrix_component = (
        benchmark_knowledge_subdomain_surface_matrix.get(
            "knowledge_subdomain_surface_matrix"
        )
        or benchmark_knowledge_subdomain_surface_matrix
        or {}
    )
    ...
    knowledge_source_drift_component = _knowledge_source_drift_component(
    ...
    knowledge_domain_release_readiness_action_plan_component = (
        _knowledge_domain_release_readiness_action_plan(
            knowledge_domain_release_readiness_action_plan_summary
        )
    )
    ...
    return {
        ...
        "knowledge_domain_capability_matrix": str(
            bundle_components.get("knowledge_domain_capability_matrix")
            or operational_components.get("knowledge_domain_capability_matrix")
        ...
        "knowledge_domain_api_surface_matrix": str(
            bundle_components.get("knowledge_domain_api_surface_matrix")
            or operational_components.get("knowledge_domain_api_surface_matrix")
            or knowledge_domain_api_surface_matrix_component.get("status")
            or (
                scorecard_components.get("knowledge_domain_api_surface_matrix") or {}
            ).get("status")
            or "unknown"
        ),
        "knowledge_domain_surface_matrix": str(
            bundle_components.get("knowledge_domain_surface_matrix")
            or operational_components.get("knowledge_domain_surface_matrix")
            or knowledge_domain_surface_matrix_component.get("status")
            or (scorecard_components.get("knowledge_domain_surface_matrix") or {}).get(
                "status"
            )
            or "unknown"
        ),
        "knowledge_domain_surface_action_plan": str(
            bundle_components.get("knowledge_domain_surface_action_plan")
            or operational_components.get("knowledge_domain_surface_action_plan")
            or knowledge_domain_surface_action_plan_component.get("status")
            or (
                scorecard_components.get("knowledge_domain_surface_action_plan") or {}
            ).get("status")
            or "unknown"
        ),
        "knowledge_subdomain_surface_matrix": str(
            bundle_components.get("knowledge_subdomain_surface_matrix")
            or operational_components.get("knowledge_subdomain_surface_matrix")
            or knowledge_subdomain_surface_matrix_component.get("status")
            or (
                scorecard_components.get("knowledge_subdomain_surface_matrix") or {}
            ).get("status")
            or "unknown"
        ),
        "knowledge_domain_capability_drift": str(
            bundle_components.get("knowledge_domain_capability_drift")
            or operational_components.get("knowledge_domain_capability_drift")
            or knowledge_domain_capability_drift_component.get("status")
            or (
                scorecard_components.get("knowledge_domain_capability_drift") or {}
            ).get("status")
            or "unknown"
        ),
        "knowledge_domain_action_plan": str(
            bundle_components.get("knowledge_domain_action_plan")
        ...
        "knowledge_domain_control_plane": str(
            bundle_components.get("knowledge_domain_control_plane")
            or operational_components.get("knowledge_domain_control_plane")
        ...
        "knowledge_domain_control_plane_drift": str(
            bundle_components.get("knowledge_domain_control_plane_drift")
            or operational_components.get("knowledge_domain_control_plane_drift")
        ...
        "knowledge_domain_release_gate": str(
            bundle_components.get("knowledge_domain_release_gate")
            or operational_components.get("knowledge_domain_release_gate")
            or knowledge_domain_release_gate_component.get("status")
            or "unknown"
        ),
        "knowledge_domain_release_readiness_action_plan": str(
            bundle_components.get("knowledge_domain_release_readiness_action_plan")
            or operational_components.get(
                "knowledge_domain_release_readiness_action_plan"
            )
            or knowledge_domain_release_readiness_action_plan_component.get("status")
            or "unknown"
        ),
        ...
        "knowledge_source_drift": str(
            bundle_components.get("knowledge_source_drift")
            or operational_components.get("knowledge_source_drift")
        ...
        "knowledge_domain_release_surface_alignment": str(
            bundle_components.get("knowledge_domain_release_surface_alignment")
...
def _artifact_rows(
    ...
    competitive_surpass_index_path: str,
    ...
    knowledge_domain_capability_matrix_path: str,
    knowledge_domain_capability_drift_path: str,
    knowledge_domain_action_plan_path: str,
    knowledge_domain_surface_action_plan_path: str,
    knowledge_subdomain_surface_matrix_path: str,
    knowledge_domain_control_plane_path: str,
    knowledge_domain_control_plane_drift_path: str,
    knowledge_domain_release_readiness_action_plan_path: str,
    ...
    knowledge_source_drift_path: str,
    ...
    return {
        ...
        "benchmark_competitive_surpass_index": row(
            "benchmark_competitive_surpass_index", competitive_surpass_index_path
        ),
        ...
        "benchmark_knowledge_domain_capability_matrix": row(
            "benchmark_knowledge_domain_capability_matrix",
            knowledge_domain_capability_matrix_path,
        ),
        "benchmark_knowledge_domain_capability_drift": row(
            "benchmark_knowledge_domain_capability_drift",
            knowledge_domain_capability_drift_path,
        ),
        "benchmark_knowledge_domain_action_plan": row(
            "benchmark_knowledge_domain_action_plan",
            knowledge_domain_action_plan_path,
        ),
        "benchmark_knowledge_domain_surface_action_plan": row(
            "benchmark_knowledge_domain_surface_action_plan",
            knowledge_domain_surface_action_plan_path,
        ),
        "benchmark_knowledge_subdomain_surface_matrix": row(
            "benchmark_knowledge_subdomain_surface_matrix",
            knowledge_subdomain_surface_matrix_path,
        ),
        "benchmark_knowledge_domain_control_plane": row(
            "benchmark_knowledge_domain_control_plane",
            knowledge_domain_control_plane_path,
        ),
        "benchmark_knowledge_domain_control_plane_drift": row(
            "benchmark_knowledge_domain_control_plane_drift",
            knowledge_domain_control_plane_drift_path,
        ),
        "benchmark_knowledge_domain_release_readiness_action_plan": row(
            "benchmark_knowledge_domain_release_readiness_action_plan",
            knowledge_domain_release_readiness_action_plan_path,
        ),
        ...
        "benchmark_knowledge_reference_inventory": row(
            ...
            knowledge_reference_inventory_path,
        ),
        ...
        "benchmark_knowledge_source_coverage": row(
            ...
            knowledge_source_coverage_path,
        ),
        "benchmark_knowledge_source_drift": row(
            "benchmark_knowledge_source_drift",
            knowledge_source_drift_path,
        ),
        ...
        "benchmark_competitive_surpass_trend": row(
            "benchmark_competitive_surpass_trend",
...
def build_companion_summary(
    *,
    ...
    benchmark_knowledge_domain_release_readiness_action_plan = (
        benchmark_knowledge_domain_release_readiness_action_plan or {}
    )
    ...
    knowledge_domain_capability_matrix_root = (
    ...
    knowledge_domain_api_surface_matrix_root = (
        benchmark_knowledge_domain_api_surface_matrix.get(
            "knowledge_domain_api_surface_matrix"
        )
        or benchmark_knowledge_domain_api_surface_matrix
    ...
    knowledge_domain_surface_matrix_root = (
        benchmark_knowledge_domain_surface_matrix.get(
            "knowledge_domain_surface_matrix"
        )
        or benchmark_knowledge_domain_surface_matrix
    ...
    knowledge_domain_surface_action_plan_root = (
        benchmark_knowledge_domain_surface_action_plan.get(
            "knowledge_domain_surface_action_plan"
        )
        or benchmark_knowledge_domain_surface_action_plan
    ...
    knowledge_subdomain_surface_matrix_root = (
        benchmark_knowledge_subdomain_surface_matrix.get(
            "knowledge_subdomain_surface_matrix"
        )
        or benchmark_knowledge_subdomain_surface_matrix
    ...
    knowledge_source_drift_root = _knowledge_source_drift_component(
        benchmark_knowledge_source_drift
    )
    ...
    knowledge_domain_api_surface_matrix_recommendations = (
        benchmark_knowledge_domain_api_surface_matrix.get("recommendations") or []
    )
    ...
    knowledge_subdomain_surface_matrix_recommendations = (
        benchmark_knowledge_subdomain_surface_matrix.get("recommendations") or []
    )
    ...
    recommendations = _compact(
        ...
        or knowledge_domain_capability_matrix_recommendations
        or knowledge_domain_api_surface_matrix_recommendations
        or knowledge_domain_surface_matrix_recommendations
        or knowledge_domain_surface_action_plan_recommendations
        or knowledge_domain_capability_drift_recommendations
        or knowledge_domain_action_plan_recommendations
        or knowledge_domain_control_plane_recommendations
        or knowledge_reference_inventory_recommendations
        or knowledge_source_action_plan_recommendations
        or knowledge_source_coverage_recommendations
        or knowledge_source_drift_recommendations
    ...
    knowledge_domain_capability_matrix_domains = (
        knowledge_domain_capability_matrix_root.get("domains") or {}
    )
    knowledge_domain_capability_matrix_priority_domains = list(
        knowledge_domain_capability_matrix_root.get("priority_domains") or []
    )
    ...
    knowledge_source_coverage_expansion_candidates = list(
        knowledge_source_coverage_root.get("expansion_candidates") or []
    )
    ...
    review_surface = (
        ...
        and component_statuses.get("knowledge_domain_capability_matrix")
        not in {
            "unknown",
            "knowledge_domain_capability_partial",
            "knowledge_domain_capability_missing",
            "blocked",
        }
    ...
    artifacts = _artifact_rows(
        ...
        artifact_paths.get("benchmark_knowledge_subdomain_surface_matrix", ""),
    ...
    return {
        ...
        "knowledge_domain_capability_matrix": knowledge_domain_capability_matrix_root,
        "knowledge_domain_capability_matrix_status": (
            knowledge_domain_capability_matrix_root.get("status") or "unknown"
        ),
        "knowledge_domain_capability_matrix_domains": (
            knowledge_domain_capability_matrix_domains
        ),
        ...
        "knowledge_domain_capability_matrix_focus_areas_detail": list(
            knowledge_domain_capability_matrix_root.get("focus_areas_detail") or []
        ),
        "knowledge_domain_capability_matrix_recommendations": _compact(
            knowledge_domain_capability_matrix_recommendations,
            limit=5,
        ),
        "knowledge_domain_api_surface_matrix": knowledge_domain_api_surface_matrix_root,
        "knowledge_domain_api_surface_matrix_status": (
            knowledge_domain_api_surface_matrix_root.get("status") or "unknown"
        ),
        "knowledge_domain_api_surface_matrix_domains": (
            knowledge_domain_api_surface_matrix_root.get("domains") or {}
        ),
        "knowledge_domain_api_surface_matrix_priority_domains": list(
            knowledge_domain_api_surface_matrix_root.get("priority_domains") or []
        ),
        "knowledge_domain_api_surface_matrix_public_api_gap_domains": list(
            knowledge_domain_api_surface_matrix_root.get("public_api_gap_domains")
            or []
        ),
        "knowledge_domain_api_surface_matrix_recommendations": _compact(
            knowledge_domain_api_surface_matrix_recommendations,
            limit=5,
        ),
        "knowledge_domain_surface_matrix": knowledge_domain_surface_matrix_root,
        "knowledge_domain_surface_matrix_status": (
            knowledge_domain_surface_matrix_root.get("status") or "unknown"
        ),
        "knowledge_domain_surface_matrix_domains": (
            knowledge_domain_surface_matrix_root.get("domains") or {}
        ),
        "knowledge_domain_surface_matrix_priority_domains": list(
            knowledge_domain_surface_matrix_root.get("priority_domains") or []
        ),
        "knowledge_domain_surface_matrix_public_surface_gap_domains": list(
            knowledge_domain_surface_matrix_root.get("public_surface_gap_domains")
            or []
        ),
        ...
        "knowledge_domain_surface_action_plan_status": (
            knowledge_domain_surface_action_plan_root.get("status") or "unknown"
        ),
        "knowledge_domain_surface_action_plan": knowledge_domain_surface_action_plan_root,
        "knowledge_domain_surface_action_plan_actions": list(
            knowledge_domain_surface_action_plan_root.get("actions") or []
        ),
        "knowledge_domain_surface_action_plan_priority_domains": list(
            knowledge_domain_surface_action_plan_root.get("priority_domains") or []
        ),
        "knowledge_domain_surface_action_plan_recommendations": _compact(
            knowledge_domain_surface_action_plan_recommendations,
            limit=5,
        ),
        "knowledge_subdomain_surface_matrix": knowledge_subdomain_surface_matrix_root,
        "knowledge_subdomain_surface_matrix_status": (
            knowledge_subdomain_surface_matrix_root.get("status") or "unknown"
        ),
        "knowledge_subdomain_surface_matrix_subdomains": (
            knowledge_subdomain_surface_matrix_root.get("subdomains") or {}
        ),
        "knowledge_subdomain_surface_matrix_priority_subdomains": list(
            knowledge_subdomain_surface_matrix_root.get("priority_subdomains") or []
        ),
        "knowledge_subdomain_surface_matrix_recommendations": _compact(
            knowledge_subdomain_surface_matrix_recommendations,
            limit=5,
        ),
        "knowledge_domain_capability_drift": knowledge_domain_capability_drift_root,
        "knowledge_domain_capability_drift_status": (
            knowledge_domain_capability_drift_root.get("status") or "unknown"
        ),
        "knowledge_domain_capability_drift_domain_regressions": list(
            knowledge_domain_capability_drift_root.get("domain_regressions") or []
        ),
        "knowledge_domain_capability_drift_domain_improvements": list(
            knowledge_domain_capability_drift_root.get("domain_improvements") or []
        ),
        ...
        "knowledge_domain_action_plan_status": (
            knowledge_domain_action_plan_root.get("status") or "unknown"
        ),
        "knowledge_domain_action_plan": knowledge_domain_action_plan_root,
        "knowledge_domain_action_plan_actions": list(
            knowledge_domain_action_plan_root.get("actions") or []
        ),
        "knowledge_domain_action_plan_priority_domains": list(
            knowledge_domain_action_plan_root.get("priority_domains") or []
        ),
        "knowledge_domain_action_plan_recommendations": _compact(
            knowledge_domain_action_plan_recommendations,
            limit=5,
        ),
        "knowledge_domain_control_plane": knowledge_domain_control_plane_root,
        "knowledge_domain_control_plane_status": (
            knowledge_domain_control_plane_root.get("status") or "unknown"
        ),
        "knowledge_domain_control_plane_domains": (
            knowledge_domain_control_plane_root.get("domains") or {}
        ),
        "knowledge_domain_control_plane_release_blockers": list(
            knowledge_domain_control_plane_root.get("release_blockers") or []
        ),
        ...
        "knowledge_domain_control_plane_drift_status": (
            knowledge_domain_control_plane_drift_root.get("status") or "unknown"
        ),
        "knowledge_domain_control_plane_drift_domain_regressions": list(
            knowledge_domain_control_plane_drift_root.get("domain_regressions") or []
        ),
        "knowledge_domain_control_plane_drift_domain_improvements": list(
            knowledge_domain_control_plane_drift_root.get("domain_improvements") or []
        ),
        ...
        "knowledge_source_coverage_status": (
            knowledge_source_coverage_root.get("status") or "unknown"
        ),
        ...
        "knowledge_source_drift": knowledge_source_drift_root,
        "knowledge_source_drift_status": (
            knowledge_source_drift_root.get("status") or "unknown"
        ),
        "knowledge_source_drift_summary": _knowledge_source_drift_summary(
            knowledge_source_drift_root
        ),
        "knowledge_source_drift_source_group_regressions": list(
            knowledge_source_drift_root.get("source_group_regressions") or []
        ),
        "knowledge_source_drift_source_group_improvements": list(
            knowledge_source_drift_root.get("source_group_improvements") or []
        ),
        "knowledge_source_drift_resolved_priority_domains": list(
            knowledge_source_drift_root.get("resolved_priority_domains") or []
        ),
        "knowledge_source_drift_new_priority_domains": list(
            knowledge_source_drift_root.get("new_priority_domains") or []
        ),
        ...
        "knowledge_domain_release_gate": knowledge_domain_release_gate,
        "knowledge_domain_release_gate_status": (
            knowledge_domain_release_gate.get("status") or "unknown"
        ),
        "knowledge_domain_release_gate_summary": (
            knowledge_domain_release_gate.get("summary") or "none"
        ),
        "knowledge_domain_release_gate_gate_open": bool(
            knowledge_domain_release_gate.get("gate_open")
        ),
        "knowledge_domain_release_gate_blocking_reasons": list(
            knowledge_domain_release_gate.get("blocking_reasons") or []
        ),
        "knowledge_domain_release_gate_warning_reasons": list(
            knowledge_domain_release_gate.get("warning_reasons") or []
        ),
        "knowledge_domain_release_gate_releasable_domains": list(
            knowledge_domain_release_gate.get("releasable_domains") or []
        ),
        ...
        "knowledge_domain_release_readiness_action_plan_status": (
            knowledge_domain_release_readiness_action_plan.get("status")
            or "unknown"
        ),
        "knowledge_domain_release_readiness_action_plan_total_action_count": (
            knowledge_domain_release_readiness_action_plan.get("total_action_count")
            or 0
        ),
        "knowledge_domain_release_readiness_action_plan_high_priority_action_count": (
            knowledge_domain_release_readiness_action_plan.get(
        ...
        "knowledge_domain_release_readiness_action_plan_gate_open": bool(
            knowledge_domain_release_readiness_action_plan.get("gate_open")
        ),
        "knowledge_domain_release_readiness_action_plan_actions": list(
            knowledge_domain_release_readiness_action_plan.get("actions") or []
        ),
        "knowledge_domain_release_readiness_action_plan_priority_domains": list(
            knowledge_domain_release_readiness_action_plan.get("priority_domains")
            or []
        ),
        "knowledge_domain_release_readiness_action_plan_recommended_first_actions": list(
            knowledge_domain_release_readiness_action_plan.get(
                "recommended_first_actions"
            )
            or []
        ),
        "knowledge_domain_release_readiness_action_plan_recommendations": _compact(
            benchmark_knowledge_domain_release_readiness_action_plan.get(
                "recommendations"
            )
            or [],
        ...
        "knowledge_reference_inventory_status": (
            knowledge_reference_inventory_root.get("status") or "unknown"
        ),
        "knowledge_reference_inventory_summary": (
            knowledge_reference_inventory_root.get("summary") or "none"
        ),
        "knowledge_reference_inventory_priority_domains": list(
            knowledge_reference_inventory_root.get("priority_domains") or []
        ),
        "knowledge_reference_inventory_total_reference_items": (
            knowledge_reference_inventory_root.get("total_reference_items") or 0
        ),
        ...
        "competitive_surpass_index": competitive_surpass_root,
        "competitive_surpass_index_status": (
            competitive_surpass_root.get("status") or "unknown"
        ),
        "competitive_surpass_primary_gaps": list(
            competitive_surpass_root.get("primary_gaps") or []
        ),
        "competitive_surpass_recommendations": _compact(
            benchmark_competitive_surpass_index.get("recommendations") or [],
            limit=5,
        ),
        "competitive_surpass_trend": competitive_surpass_trend_root,
        "competitive_surpass_trend_status": (
            competitive_surpass_trend_root.get("status") or "unknown"
        ),
        "competitive_surpass_trend_summary": _competitive_surpass_trend_summary(
            competitive_surpass_trend_root
        ),
        "competitive_surpass_trend_score_delta": (
            competitive_surpass_trend_root.get("score_delta") or 0
        ),
        "competitive_surpass_trend_pillar_improvements": list(
            competitive_surpass_trend_root.get("pillar_improvements") or []
        ),
        "competitive_surpass_trend_pillar_regressions": list(
            competitive_surpass_trend_root.get("pillar_regressions") or []
        ),
        "competitive_surpass_trend_resolved_primary_gaps": list(
            competitive_surpass_trend_root.get("resolved_primary_gaps") or []
        ),
        "competitive_surpass_trend_new_primary_gaps": list(
            competitive_surpass_trend_root.get("new_primary_gaps") or []
        ),
...
def render_markdown(payload: Dict[str, Any]) -> str:
    ...
    if capability_recommendations:
        for item in capability_recommendations:
            lines.append(f"- recommendation: {item}")
    lines.extend(["", "## Knowledge Domain API Surface Matrix", ""])
    lines.append(
        "- `status`: "
        f"`{payload.get('knowledge_domain_api_surface_matrix_status') or 'unknown'}`"
    )
    api_surface_domains = payload.get("knowledge_domain_api_surface_matrix_domains") or {}
    if api_surface_domains:
        for name, row in api_surface_domains.items():
            lines.append(
                "- "
                f"`{name}` "
                f"status=`{row.get('status')}` "
    ...
    api_surface_recommendations = (
        payload.get("knowledge_domain_api_surface_matrix_recommendations") or []
    )
    if api_surface_recommendations:
        for item in api_surface_recommendations:
            lines.append(f"- recommendation: {item}")
    lines.extend(["", "## Knowledge Domain Surface Matrix", ""])
    lines.append(
        "- `status`: "
        f"`{payload.get('knowledge_domain_surface_matrix_status') or 'unknown'}`"
    )
    surface_domains = payload.get("knowledge_domain_surface_matrix_domains") or {}
    if surface_domains:
        for name, row in surface_domains.items():
            lines.append(
                "- "
                f"`{name}` "
                f"status=`{row.get('status')}` "
                f"public=`{row.get('public_surface_status')}` "
                f"reference_items=`{row.get('reference_item_count')}` "
                f"subcapabilities=`{row.get('total_subcapability_count')}`"
            )
    else:
        lines.append("- none")
    surface_recommendations = (
        payload.get("knowledge_domain_surface_matrix_recommendations") or []
    )
    ...
    lines.append(
        "- `status`: "
        f"`{payload.get('knowledge_domain_surface_action_plan_status') or 'unknown'}`"
    )
    ...
    if surface_action_plan_recommendations:
        for item in surface_action_plan_recommendations:
            lines.append(f"- recommendation: {item}")
    lines.extend(["", "## Knowledge Subdomain Surface Matrix", ""])
    lines.append(
        "- `status`: "
        f"`{payload.get('knowledge_subdomain_surface_matrix_status') or 'unknown'}`"
    )
    subdomains = payload.get("knowledge_subdomain_surface_matrix_subdomains") or {}
    if subdomains:
        for name, row in list(subdomains.items())[:6]:
            lines.append(
                "- "
                f"`{name}` "
                f"status=`{row.get('status')}` "
                f"present_routes=`{row.get('present_route_count')}` "
                f"expected_routes=`{row.get('expected_route_count')}`"
            )
    else:
        lines.append("- none")
    subdomain_recommendations = (
        payload.get("knowledge_subdomain_surface_matrix_recommendations") or []
    )
    if subdomain_recommendations:
        for item in subdomain_recommendations:
            lines.append(f"- recommendation: {item}")
    ...
    if knowledge_reference_domains:
        ...
        lines.append("- none")
    focus_tables = knowledge_reference_inventory.get("focus_tables_detail") or []
    if focus_tables:
    ...
    knowledge_gate = payload.get("knowledge_domain_release_gate") or {}
    lines.append(f"- `status`: `{knowledge_gate.get('status') or 'unknown'}`")
    lines.append(f"- `summary`: {knowledge_gate.get('summary') or 'none'}")
    lines.append(f"- `gate_open`: `{bool(knowledge_gate.get('gate_open'))}`")
    lines.append(
        "- `blocked_domains`: "
        + (
            ", ".join(str(item) for item in (knowledge_gate.get("blocked_domains") or []))
            or "none"
        )
    )
    lines.append(
        ...
        + (
            ", ".join(
                str(item) for item in (knowledge_gate.get("releasable_domains") or [])
            )
            or "none"
        )
    )
    lines.extend(["", "## Knowledge Domain Release Readiness Action Plan", ""])
    readiness_plan = (
        payload.get("knowledge_domain_release_readiness_action_plan") or {}
    )
    lines.append(f"- `status`: `{readiness_plan.get('status') or 'unknown'}`")
    lines.append(
        "- `total_action_count`: "
        f"`{readiness_plan.get('total_action_count') or 0}`"
    )
    lines.append(
        "- `priority_domains`: "
        + (
    ...
    for item in (
        payload.get("knowledge_domain_release_readiness_action_plan_recommendations")
        or []
    ):
        lines.append(f"- recommendation: {item}")
...
def main() -> None:
    ...
    parser.add_argument("--benchmark-knowledge-subdomain-surface-matrix", default="")
    ...
    artifact_paths = {
        ...
        "benchmark_knowledge_domain_api_surface_matrix": (
            args.benchmark_knowledge_domain_api_surface_matrix
        ),
        "benchmark_knowledge_domain_surface_matrix": (
        ...
        "benchmark_knowledge_subdomain_surface_matrix": (
            args.benchmark_knowledge_subdomain_surface_matrix
        ),
        "benchmark_knowledge_domain_capability_drift": (
        ...
        "benchmark_knowledge_domain_release_readiness_action_plan": (
            args.benchmark_knowledge_domain_release_readiness_action_plan
        ),
        "benchmark_knowledge_domain_release_surface_alignment": (
    ...
    payload = build_companion_summary(
        ...
        benchmark_knowledge_domain_api_surface_matrix=_maybe_load_json(
            args.benchmark_knowledge_domain_api_surface_matrix
        ),
        ...
        benchmark_knowledge_subdomain_surface_matrix=_maybe_load_json(
            args.benchmark_knowledge_subdomain_surface_matrix
        ),
        ...
        benchmark_knowledge_domain_release_readiness_action_plan=_maybe_load_json(
            args.benchmark_knowledge_domain_release_readiness_action_plan
        ),
