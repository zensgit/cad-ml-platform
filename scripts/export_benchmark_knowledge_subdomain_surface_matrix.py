#!/usr/bin/env python3
"""Export benchmark knowledge subdomain surface matrix signals."""
...
def build_summary(*, title: str, artifact_paths: Dict[str, str]) -> Dict[str, Any]:
    component = build_knowledge_subdomain_surface_matrix()
    return {
        "title": title,
        "generated_at": int(time.time()),
        "knowledge_subdomain_surface_matrix": component,
        "recommendations": knowledge_subdomain_surface_matrix_recommendations(component),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Export benchmark knowledge subdomain surface matrix signals."
    )
    parser.add_argument(
        "--title",
        default="Benchmark Knowledge Subdomain Surface Matrix",
    )
    ...
    if args.output_md:
        _write_output(
            args.output_md,
            render_knowledge_subdomain_surface_matrix_markdown(payload, args.title),
        )
    print(rendered)
