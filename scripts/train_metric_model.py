...
def _default_feedback_log_path() -> str:
    return os.getenv("FEEDBACK_LOG_PATH", "data/feedback_log.jsonl")


def _iter_feedback_paths(data_dir: str) -> List[str]:
    paths = sorted(glob.glob(os.path.join(data_dir, "*.jsonl")))
    fallback_path = _default_feedback_log_path()
    if os.path.isfile(fallback_path) and fallback_path not in paths:
        paths.append(fallback_path)
    return paths


def _resolve_entry_label(entry: Dict[str, Any]) -> Optional[str]:
    for field_name in (
        "correct_label",
...
def _resolve_anchor_id(entry: Dict[str, Any]) -> Optional[str]:
    for field_name in ("analysis_id", "doc_id"):
        value = entry.get(field_name)
        text = str(value or "").strip()
        if text:
            return text
...
def load_feedback_data(data_dir: str) -> List[Dict[str, Any]]:
    ...
    files = _iter_feedback_paths(data_dir)
    dataset = []
    logger.info(f"Found {len(files)} feedback log files for metric training")
...
def prepare_training_triplets(feedback_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for entry in feedback_data:
        anchor_id = _resolve_anchor_id(entry)
        correct_label = _resolve_entry_label(entry)
        if not anchor_id or not correct_label:
            continue
        ...
        # Hard Negative Mining: If user flagged as hard negative, try to find a
        # sample from the originally predicted class or a very similar class.
        # For now, we do not have the original prediction here unless feedback
        # logs provide it, so we fall back to random negatives.
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Metric Learning Model from Feedback"
    )
    parser.add_argument(
        "--data-dir",
        default="data/feedback",
        help="Directory containing feedback logs",
    )
    parser.add_argument(
        "--output-model",
        default="models/metric_learning/fine_tuned.pth",
        help="Path to save model",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Simulate training without saving",
    )
    parser.add_argument(
        "--mock-data",
        action="store_true",
