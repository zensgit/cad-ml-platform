#!/usr/bin/env python3
"""Export benchmark knowledge source coverage drift against a previous baseline."""
...
def build_knowledge_source_drift_summary(
    *,
    title: str,
    ...
    component = build_knowledge_source_drift_status(current_summary, previous_summary)
    return {
        "title": title,
        "generated_at": int(time.time()),
        "knowledge_source_drift": component,
        "recommendations": knowledge_source_drift_recommendations(component),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Export benchmark knowledge source drift summary."
    )
    parser.add_argument("--title", default="Benchmark Knowledge Source Drift")
    ...
    payload = build_knowledge_source_drift_summary(
        title=args.title,
    ...
    if args.output_md:
        _write_output(
            args.output_md,
            render_knowledge_source_drift_markdown(payload, args.title),
        )
    print(rendered)
