#!/usr/bin/env python3
"""Export benchmark knowledge domain capability drift against a previous baseline."""
...
def build_summary(
    *,
    ...
    return {
        ...
        "knowledge_domain_capability_drift": component,
        "recommendations": knowledge_domain_capability_drift_recommendations(component),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Export benchmark knowledge domain capability drift summary."
    )
    parser.add_argument(
        "--title",
        default="Benchmark Knowledge Domain Capability Drift",
    )
    ...
    if args.output_md:
        _write_output(
            args.output_md,
            render_knowledge_domain_capability_drift_markdown(payload, args.title),
        )
    print(rendered)
