...
def _load_stack_status(stack_summary_json: str, index_json: str) -> Dict:
    """Load eval reporting stack status from summary and index JSON files.

    Returns a dict with keys: available, status, missing_count, stale_count,
    mismatch_count, landing_page, static_report, interactive_report.
    """
    fallback: Dict = {
        "available": False,
        "status": "unavailable",
    ...
    ix = None
    try:
        if stack_summary_json:
            p = Path(stack_summary_json)
            if p.exists():
    ...
    return {
        "available": True,
        "status": str(ss.get("status") or "unknown"),
        "missing_count": int(ss.get("missing_count", 0) or 0),
        "stale_count": int(ss.get("stale_count", 0) or 0),
        "mismatch_count": int(ss.get("mismatch_count", 0) or 0),
        "landing_page": str((ix or {}).get("landing_page_html") or ""),
        "static_report": str(ss.get("static_report_html") or ""),
        "interactive_report": str(ss.get("interactive_report_html") or ""),
    }
...
def main():
    ...
    parser.add_argument("--stack-summary-json", default="",
                        help="Path to eval_reporting_stack_summary.json")
    parser.add_argument("--index-json", default="",
                        help="Path to eval_reporting_index.json")
    ...
    # Load stack status (additive — missing is ok)
    stack_status = _load_stack_status(
        str(args.stack_summary_json).strip(),
        str(args.index_json).strip(),
    )
    ...
    for channel in channels:
        if channel == "slack":
            ...
            if stack_status["available"]:
                stack_line = (
                    f"\nEval Reporting Stack: status={stack_status['status']}, "
                    f"missing={stack_status['missing_count']}, "
                    f"stale={stack_status['stale_count']}, "
                    f"mismatch={stack_status['mismatch_count']}\n"
                )
                text_body += stack_line
                html_body += f"<p>{stack_line.strip()}</p>"
