#!/usr/bin/env python3
"""Export benchmark knowledge domain validation matrix signals."""
...
def build_summary(*, title: str, artifact_paths: Dict[str, str]) -> Dict[str, Any]:
    ...
    return {
        ...
        "knowledge_domain_validation_matrix": component,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Export benchmark knowledge domain validation matrix signals."
    )
    parser.add_argument(
        "--title",
        default="Benchmark Knowledge Domain Validation Matrix",
    )
    ...
    if args.output_md:
        _write_output(
            args.output_md,
            render_knowledge_domain_validation_matrix_markdown(payload, args.title),
        )
    print(rendered)
