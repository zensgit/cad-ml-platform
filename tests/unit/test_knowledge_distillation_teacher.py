...
from src.ml.knowledge_distillation import TeacherModel  # noqa: E402


def test_titleblock_teacher_uses_file_bytes(monkeypatch) -> None:
    label_map = {"人孔": 0, "other": 1}
    teacher = TeacherModel(
        teacher_type="titleblock", label_to_idx=label_map, num_classes=2
    )
    ...
    logits = teacher.generate_soft_labels(
        ["masked.dxf"], file_bytes_list=[b"dxf-bytes"]
    )
...
def test_titleblock_teacher_unknown_label_falls_back_to_other(monkeypatch) -> None:
    label_map = {"other": 0}
    teacher = TeacherModel(
        teacher_type="titleblock", label_to_idx=label_map, num_classes=1
    )
    ...
    monkeypatch.setattr(
        "src.ml.titleblock_extractor.TitleBlockClassifier.predict",
        lambda _self, _entities, bbox=None: {
            "label": "不存在的标签",
            "confidence": 0.9,
        },
        raising=True,
...
def test_titleblock_teacher_normalizes_labels_to_buckets(monkeypatch) -> None:
    label_map = {"法兰": 0, "other": 1}
    teacher = TeacherModel(
        teacher_type="titleblock", label_to_idx=label_map, num_classes=2
    )
    ...
    monkeypatch.setattr(
        "src.ml.titleblock_extractor.TitleBlockClassifier.predict",
        # Teacher predicts a fine label; student label-space contains only buckets.
        lambda _self, _entities, bbox=None: {"label": "对接法兰", "confidence": 0.9},
        raising=True,
    )
