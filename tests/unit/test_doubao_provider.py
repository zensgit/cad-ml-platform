"""Tests for Doubao Vision (豆包视觉) Provider.

Tests the ByteDance Volcengine Ark API integration for vision analysis.
"""
...
class TestDoubaoVisionProviderInit:
    """Tests for DoubaoVisionProvider initialization."""

    def test_init_with_api_key(self) -> None:
        """Test initialization with explicit API key."""
        provider = DoubaoVisionProvider(api_key="test-key")
        assert provider.api_key == "test-key"
        assert provider.model == "doubao-1-5-vision-pro-32k-250115"
        assert provider.provider_name == "doubao"

    def test_init_with_ark_env(self) -> None:
        """Test initialization with ARK_API_KEY env var."""
        with patch.dict(os.environ, {"ARK_API_KEY": "ark-env-key"}):
            provider = DoubaoVisionProvider()
            assert provider.api_key == "ark-env-key"

    def test_init_with_volcengine_env(self) -> None:
        """Test initialization with VOLCENGINE_API_KEY env var."""
        with patch.dict(os.environ, {"VOLCENGINE_API_KEY": "volc-env-key"}, clear=True):
            # Clear other keys to test fallback
            os.environ.pop("ARK_API_KEY", None)
            os.environ.pop("DOUBAO_API_KEY", None)
            provider = DoubaoVisionProvider()
            assert provider.api_key == "volc-env-key"

    def test_init_with_doubao_env(self) -> None:
        """Test initialization with DOUBAO_API_KEY env var."""
        with patch.dict(os.environ, {"DOUBAO_API_KEY": "doubao-env-key"}, clear=True):
            # Clear other keys to test fallback
            os.environ.pop("ARK_API_KEY", None)
            os.environ.pop("VOLCENGINE_API_KEY", None)
            provider = DoubaoVisionProvider()
            assert provider.api_key == "doubao-env-key"
    ...
    def test_init_with_custom_model(self) -> None:
        ...
        provider = DoubaoVisionProvider(api_key="test-key", model="doubao-1.5-vision-lite-250315")
        assert provider.model == "doubao-1.5-vision-lite-250315"

    def test_init_with_chinese_prompt(self) -> None:
        """Test Chinese prompt is used by default."""
        provider = DoubaoVisionProvider(api_key="test-key", use_chinese_prompt=True)
        assert "工程图纸分析专家" in provider.system_prompt

    def test_init_with_english_prompt(self) -> None:
        """Test English prompt option."""
        provider = DoubaoVisionProvider(api_key="test-key", use_chinese_prompt=False)
        assert "engineering drawing analyzer" in provider.system_prompt

    def test_init_with_custom_prompt(self) -> None:
        """Test custom system prompt."""
        custom_prompt = "Custom analysis prompt"
        provider = DoubaoVisionProvider(api_key="test-key", system_prompt=custom_prompt)
        assert provider.system_prompt == custom_prompt

    def test_init_default_base_url(self) -> None:
        """Test default base URL is Ark endpoint."""
        provider = DoubaoVisionProvider(api_key="test-key")
        assert "ark.cn-beijing.volces.com" in provider.base_url

    def test_init_custom_base_url(self) -> None:
        """Test custom base URL."""
        provider = DoubaoVisionProvider(api_key="test-key", base_url="https://custom.api.com/v3")
        assert provider.base_url == "https://custom.api.com/v3"

    def test_init_timeout_configuration(self) -> None:
        """Test timeout configuration."""
        provider = DoubaoVisionProvider(api_key="test-key", timeout_seconds=180.0)
        assert provider.timeout_seconds == 180.0

    def test_init_temperature_configuration(self) -> None:
        """Test temperature configuration."""
        provider = DoubaoVisionProvider(api_key="test-key", temperature=0.5)
        assert provider.temperature == 0.5


class TestDoubaoVisionProviderAnalyze:
    """Tests for DoubaoVisionProvider.analyze_image method."""
...
class TestDoubaoVisionProviderImageDetection:
...
class TestDoubaoVisionProviderJsonExtraction:
...
class TestDoubaoVisionFactoryFunction:
    """Tests for create_doubao_provider factory function."""

    def test_create_with_defaults(self) -> None:
        """Test factory with default settings."""
        with patch.dict(os.environ, {"ARK_API_KEY": "test-key"}):
            provider = create_doubao_provider()
            assert provider.model == "doubao-1-5-vision-pro-32k-250115"
            assert provider.use_chinese_prompt is True

    def test_create_with_custom_model(self) -> None:
        """Test factory with custom model."""
        provider = create_doubao_provider(api_key="test-key", model="doubao-1.5-vision-lite-250315")
        assert provider.model == "doubao-1.5-vision-lite-250315"

    def test_create_with_english_prompt(self) -> None:
        """Test factory with English prompt."""
        provider = create_doubao_provider(api_key="test-key", use_chinese_prompt=False)
        assert "engineering drawing" in provider.system_prompt


class TestFactoryIntegration:
    """Tests for Doubao Vision integration in factory.py."""

    def test_doubao_in_provider_registry(self) -> None:
        """Test Doubao Vision is in provider registry."""
        assert "doubao" in PROVIDER_REGISTRY
        assert "doubao_vision" in PROVIDER_REGISTRY
        assert "doubao-vision" in PROVIDER_REGISTRY
        assert "bytedance" in PROVIDER_REGISTRY
        assert "volcengine" in PROVIDER_REGISTRY
        assert "ark" in PROVIDER_REGISTRY

    def test_doubao_in_factory_registry(self) -> None:
        """Test Doubao Vision is in factory registry."""
        assert "doubao" in FACTORY_REGISTRY
        assert "doubao_vision" in FACTORY_REGISTRY
        assert "doubao-vision" in FACTORY_REGISTRY
        assert "bytedance" in FACTORY_REGISTRY
        assert "volcengine" in FACTORY_REGISTRY
        assert "ark" in FACTORY_REGISTRY

    def test_create_doubao_via_factory(self) -> None:
        """Test creating Doubao Vision via main factory."""
        provider = create_vision_provider("doubao", api_key="test-key")
        assert provider.provider_name == "doubao"

    def test_create_doubao_vision_via_factory(self) -> None:
        """Test creating Doubao Vision via doubao_vision alias."""
        provider = create_vision_provider("doubao_vision", api_key="test-key")
        assert provider.provider_name == "doubao"

    def test_create_bytedance_via_factory(self) -> None:
        """Test creating Doubao Vision via bytedance alias."""
        provider = create_vision_provider("bytedance", api_key="test-key")
        assert provider.provider_name == "doubao"

    def test_create_volcengine_via_factory(self) -> None:
        """Test creating Doubao Vision via volcengine alias."""
        provider = create_vision_provider("volcengine", api_key="test-key")
        assert provider.provider_name == "doubao"

    def test_auto_detect_with_ark_key(self) -> None:
    ...
    def test_auto_detect_with_volcengine_key(self) -> None:
    ...
    def test_get_available_providers_includes_doubao(self) -> None:
        """Test get_available_providers includes Doubao Vision."""
        providers = get_available_providers()
        assert "doubao" in providers
        assert providers["doubao"]["description"]
        assert "豆包视觉" in providers["doubao"]["description"]
        assert "0.003" in providers["doubao"]["description"]  # Price mention


class TestDoubaoVisionProviderClose:
