...
from scripts.export_benchmark_knowledge_domain_surface_action_plan import build_summary
...
def _surface_matrix() -> dict:
    return {
        "knowledge_domain_surface_matrix": {
            "status": "knowledge_domain_surface_matrix_partial",
            "domains": {
                "standards": {
        ...
        "recommendations": ["Expose bearing lookup through stable public routes."],
    }


def _surface_action_plan() -> dict:
    return build_summary(
        title="Benchmark Knowledge Domain Surface Action Plan",
        knowledge_domain_surface_matrix_summary=_surface_matrix(),
        artifact_paths={
...
def test_build_bundle_and_companion_include_surface_action_plan_passthrough() -> None:
    surface_action_plan = _surface_action_plan()
    bundle = build_bundle(
        title="Bundle",
        ...
        benchmark_knowledge_domain_surface_action_plan=surface_action_plan,
        feedback_flywheel={},
        assistant_evidence={},
        review_queue={},
        ocr_review={},
        artifact_paths={
            "benchmark_knowledge_domain_surface_action_plan": "surface-action-plan.json",
        },
    )
    ...
    assert bundle["knowledge_domain_surface_action_plan_status"] in {
        "knowledge_domain_surface_action_plan_partial",
        "knowledge_domain_surface_action_plan_blocked",
    }
    assert (
        bundle["component_statuses"]["knowledge_domain_surface_action_plan"]
        == bundle["knowledge_domain_surface_action_plan_status"]
    )
    assert bundle["artifacts"]["benchmark_knowledge_domain_surface_action_plan"][
        "present"
    ] is True
    assert companion["knowledge_domain_surface_action_plan_actions"][0][
        "subcapability"
    ] == "bearing_lookup"
    assert any(
        "bearing lookup" in item.lower()
        for item in companion["knowledge_domain_surface_action_plan_recommendations"]
    )


def test_build_release_surfaces_include_surface_action_plan_review_signals() -> None:
    surface_action_plan = _surface_action_plan()
    ...
    assert decision["knowledge_domain_surface_action_plan_actions"][0][
        "subcapability"
    ] == "bearing_lookup"
    assert any(
        "bearing lookup" in item.lower() for item in decision["review_signals"]
    )
    assert runbook["knowledge_domain_surface_action_plan_actions"][0][
        "subcapability"
    ] == "bearing_lookup"
    assert any(
        "bearing lookup" in item.lower() for item in runbook["review_signals"]
    )
