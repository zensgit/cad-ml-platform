...
def test_plate_aux_does_not_collapse_into_plate() -> None:
    """Discriminator #4 negative half: matchers are anchored — plate_aux ≠ plate."""
    assert te.normalized_family("plate.dxf") == "plate"
    assert te.normalized_family("plate_aux.dxf") == "plate_aux"
    assert te.normalized_family("plate_aux.dxf") != te.normalized_family("plate.dxf")
...
def test_discriminator_14_gate_floor_still_fail_closed() -> None:
    """Design-lock discriminator #14: gate has no pass path; auto_retrain still blocked."""
    from scripts import eval_integrity_gate as gate

    with pytest.raises(gate.GateBlocked):
        gate.check()
    # CLI still exits non-zero (no argv opens a pass path).
    assert gate.main([]) != 0

    auto = Path("scripts/auto_retrain.sh")
    assert auto.is_file()
    text = auto.read_text(encoding="utf-8")
    assert "eval_integrity_gate" in text
