"""reload_model behavior coverage (success / not_found / size / version / rollback / errors).

Phase-A seal: POST /api/v1/model/reload is fail-closed (always 403, see
tests/unit/test_retrain_bypass_seals.py), so these tests exercise the loader DIRECTLY via
``reload_model(...)`` — the route's old JSON envelope mirrored this result dict field-for-field,
so every behavioral assertion is preserved; only the HTTP transport is gone.
"""
import os
import pickle
