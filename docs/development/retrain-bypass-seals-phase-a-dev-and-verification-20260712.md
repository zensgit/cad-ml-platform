...
| `src/core/model/hot_reload.py::HotReloadManager.load_model` | an UNMOUNTED arbitrary-path hot-reload loader (`await self.loader.load(path)`) with no importer today | registered as unmounted scaffold; must gain the same gate treatment if ever mounted to a router — the "automated retrain/hot-reload paths are sealed" banner covers only the mounted set, and this closes the enumeration gap |
